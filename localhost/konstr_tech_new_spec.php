...
if(isset($_POST['submit'])){
	$QUERY = "INSERT INTO specifications (Comments, Proj_id, Spec_date) VALUES ('".$_POST['comment']."', ".$_GET["n"].", '".date("Y-m-d")."')";
	mysqli_query($link, $QUERY) or die("Ошибка 1 ".mysqli_error($link));
	
	$QUERY = "SELECT MAX(Spec_id) FROM specifications WHERE Proj_id=".$_GET["n"];
	$result = mysqli_query($link, $QUERY) or die("Ошибка 2".mysqli_error($link));
	$spec_id = mysqli_fetch_row($result);
	$spec_id = $spec_id[0];
	...
	$_FILES['userfile']['name'] = "spec_n".$_GET["n"].substr($my_string, $point_place, $type_len);
	$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
	//echo $uploadfile;
	move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile);
	
	$QUERY = "UPDATE specifications SET Link='".$uploadfile."' WHERE Spec_id =".$spec_id;
...
<head>
	...
	<title>Спецификация для заказа</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Спецификация для заказа №<?echo $_GET["n"];?></p>
						</div>
					</div>
<!--############################################################################# -->
