...
if(isset($_POST['submit']))
{
	...
	SET 
		Prov_tel="'.$_POST['tel_num'].'",
		Cont_prov_name="'.$_POST['cont_name'].'",
		E_mail="'.$_POST['mail'].'",
		Kpp='.$_POST['kpp'].',
		OGRN='.$_POST['ogrn'].',
		Check_acc='.$_POST['balance'].',
		ITN='.$_POST['inn'].',
		Addr="'.form_control($_POST['address']).'",
		Comments="'.$_POST['comment'].'" 
	WHERE
		Prov_id='.$_GET["prov"];
	mysqli_query($link, $QUERY) or die("Ошибка submit".mysqli_error($link));
	header("Location: konstr_tech_purchase?n=".$_GET["n"]);
}
		...
		<div class="row justify-content-md-center" style="width:1080px;">
			...
			<div class="col-fluid" >
				<div class="side-bar" style="width: 100px; height:661px;">
					...
					<div class="row-fluid">
						<div class="col text-center nopadding">
							<a href="konstr_tech_projs"><img src="img/proj_sb_a.png" class="img"></a>
						</div>
					</div>
				</div>
			</div>
											...
											<form action="" method="POST">
												...
												<div class="row align-items-center ml-2" style="padding-top: 3rem;">
													<div class="col-md-2 pl-0 pr-0" >
														<button hidden class="btn btn-primary btn-block btn-sm ml-0" type="submit" name="submit"><small>Сохранить</small></button>
													</div>
													<div class="col-md-2 pl-0 ml-4 pr-0">
														<button class="btn btn-secondary btn-block btn-sm ml-0" id="change_button"><small>Изменить</small></button>
													</div>
												</div>
											</form>
										</div>
...
<div tabindex="-1" class="modal fade" id="exampleModal2" role="dialog" aria-hidden="true" aria-labelledby="exampleModalLabel2" style="display: none;">
	...
	<script>
		document.getElementById("change_button").addEventListener("click", function(event){
			...
			$(".form-control").attr("style", "border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;");
