...
if(isset($_POST["smeta_approved"])){
	$QUERY = "UPDATE pr_contract_docs SET Pr_doc_date='".date("Y-m-d")."' WHERE Pr_doc_id=".$smeta[3];
	mysqli_query($link, $QUERY) or die("Ошибка 4 ".mysqli_error($link));
	header("Location: gen_dir_cont_prep?n=".$_GET["n"]);
}
if(isset($_POST["smeta_comment_submit"])){
	$QUERY = "UPDATE pr_contract_docs SET Comments='".$_POST["smeta_comment"]."' WHERE Pr_doc_id=".$smeta[3];
	mysqli_query($link, $QUERY) or die("Ошибка 5 ".mysqli_error($link));
	header("Location: gen_dir_cont_prep?n=".$_GET["n"]);
}
if(isset($_POST["plan_comment_submit"])){
	$QUERY = "UPDATE pr_contract_docs SET Comments='".$_POST["plan_comment"]."' WHERE Pr_doc_id=".$plan[3];
	mysqli_query($link, $QUERY) or die("Ошибка 6 ".mysqli_error($link));
	header("Location: gen_dir_cont_prep?n=".$_GET["n"]);
}
?>
							...
							<div class="tab-content" id="myTabContent" >
								<div class="tab-pane fade active show" id="contract" role="tabpanel" aria-labelledby="contract-tab" >
									<div class="row-fluid mt-4 mr-3">
										<div class="col nopadding">
											<?if (!$contract){
												echo '<button class="btn btn-primary btn-sm ml-0" onClick=\'location.href="gen_dir_new_contract?n='.$_GET["n"].'"\'><small>Добавить договор</small></button>';
											}
											else{
											...
											<div class="row mt-4 mr-5 align-items-center" style="">
												...
												<div class="col-md-2 " name="cust_id">
													<button class="btn btn-secondary btn-sm btn-block m-0" onClick=\'location.href="gen_dir_edit_contract?n='.$_GET["n"].'"\'><small>Изменить</small></button>
												</div>
											</div>';
											}?>
								...
								<div class="tab-pane fade" id="tech_docs" role="tabpanel" aria-labelledby="tech_docs-tab">
									<div class="row-fluid mt-4 mr-3">
										<div class="col nopadding">
<?if (!$tech_doc){
	echo '<button class="btn btn-primary btn-sm ml-0" onClick=\'location.href="gen_dir_new_technic_doc?n='.$_GET["n"].'"\'></small>Добавить<small></button>';
}
else 
{
	echo '											
	...
	//	echo $rows;
		for ($i = 0 ; $i < $rows ; ++$i)
		{
			...
			echo "<td><a href='gen_dir_edit_technic_doc?n=".$_GET["n"]."&doc=".$row[0]."'>Ред.</a></td>";
			echo "</tr>";
		}
    ...
    echo '                 </tbody>
			...
			<button class="col-md-2 btn btn-primary btn-block btn-sm ml-0" onClick=\'location.href="gen_dir_new_technic_doc?n='.$_GET["n"].'"\' style="margin-left: 2em"><small>Добавить</small></button>';
}?>
										</div>
									</div>
								</div>
									...
									<div class="row-fluid mt-4 mr-3">
										<div class="col nopadding">
											...
											<div class="row mt-3 mr-5 align-items-center" style="">
												...
												<div class="col-md-4"><input disabled class="form-control" id="plan_comment" name="plan_comment" type="text" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(0,0,0,.10); font-size: small;"  placeholder="добавьте комментарий" value="'.$plan[1].'"></div>
												
												<div class="col-md-2">
													<button	class="btn btn-secondary btn-sm btn-block m-0 pl-0 pr-0" type="button" id="change_button2"><small>Изменить</small></button>
												</div>
												<div class="col-md-2">
													<button	class="btn btn-primary btn-sm btn-block m-0 pl-0 pr-0" type="submit" name="plan_comment_submit"><small>Сохранить</small></button>
												</div>
												</form>	
											...
											<div class="row mt-3 mr-5 align-items-center" style="">
												...
												<div class="col-md-2">
													...
													if ($approved!="Нет") echo "style='display: none;'"; 
													echo 'class="btn btn-success btn-sm btn-block m-0 pl-0 pr-0 pr-0" type="submit" name="plan_approved"><small>Согласовать</small></button></form>
												</div>
									...
									<div class="row-fluid mt-4 mr-3">
										<div class="col nopadding">
											...
											<div class="row mt-3 mr-5 align-items-center" style="">
												...
												<div class="col-md-4"><input disabled class="form-control" id="smeta_comment" name="smeta_comment" type="text" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(0,0,0,.10); font-size: small;"  placeholder="добавьте комментарий" value="'.$smeta[1].'"></div>
												
												<div class="col-md-2">
													<button	class="btn btn-secondary btn-sm btn-block m-0 pl-0 pr-0" type="button" id="change_button3"><small>Изменить</small></button>
												</div>
												<div class="col-md-2">
													<button	class="btn btn-primary btn-sm btn-block m-0 pl-0 pr-0" type="submit" name="smeta_comment_submit"><small>Сохранить</small></button>
												</div>
												</form>	
											...
											<div class="row mt-3 mr-5 align-items-center" style="">
												...
												<div class="col-md-2">
													<form method="POST"><button ';
													if ($approved!="Нет") echo "style='display: none;'"; 
													echo 'class="btn btn-success btn-sm btn-block m-0 pl-0 pr-0 pr-0" type="submit" name="smeta_approved"><small>Согласовать</small></button></form>
												</div>
...
<div tabindex="-1" class="modal fade" id="exampleModal2" role="dialog" aria-hidden="true" aria-labelledby="exampleModalLabel2" style="display: none;">
	...
	<script>
		...
		document.getElementById("change_button3").addEventListener("click", function(event){
			event.preventDefault();
			$("#smeta_comment").removeAttr("disabled");
			$("#smeta_comment").attr("style", "border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;");
