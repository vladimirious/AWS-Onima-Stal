...
$QUERY= "SELECT Comments, Link FROM techn_doc WHERE Techn_doc_id=".$_GET["doc"];
...
if(isset($_POST['submit']))
{
	...
	//echo $my_string."<BR>";
		...
		$_FILES['userfile']['name'] = "techn_doc_n".$_GET["doc"].substr($my_string, $point_place, $type_len);
		$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
	//echo $uploadfile;
		if(move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile))
		{
			$QUERY = "UPDATE techn_doc SET Comments='".$_POST['comment']."', Link='".$uploadfile."' WHERE Techn_doc_id=".$_GET["doc"]." AND Proj_id=".$_GET["n"];
			mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
			header("Location: konstr_tech_ktp?n=".$_GET["n"]);
			exit;
		}
		else echo "Ошибка!";
	}
	else{
			$QUERY = "UPDATE techn_doc SET Comments='".$_POST['comment']."' WHERE Techn_doc_id=".$_GET["doc"]." AND Proj_id=".$_GET["n"];
			mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
			header("Location: konstr_tech_ktp?n=".$_GET["n"]);
			exit;
	}
...
<head>
	...
	<title>Редактировать технологический документ</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Редактировать технологический документ</p>
						</div>
					</div>
<!--############################################################################# -->
					...
					<div class="row mt-4 mr-5 align-items-center" style="">
						...
						<label class="col" for="upload-file" id="upload-file-label" style="height:35px; border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small; padding-left: 1rem; padding-top:8px;">Обзор...</label>
					...
					<div class="row mt-4 mr-5 align-items-center" style="">
						...
						<input required class="col form-control" id="username" name="comment" type="text" value="<?echo $row[0];?>" placeholder="комментарий" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;" >
						<div class="col-md-2 ml-4" style=""></div>
					</div>
