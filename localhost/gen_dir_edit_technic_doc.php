...
$QUERY = "SELECT Link, Comments, Type_doc FROM tech_doc WHERE Tech_doc_id=".$_GET['doc']." AND Proj_id=".$_GET["n"];
...
if(isset($_POST['delete'])){
		...
		$QUERY = "DELETE FROM tech_doc WHERE Tech_doc_id =".$_GET["doc"];
		mysqli_query($link, $QUERY) or die("Ошибка удаления ".mysqli_error($link));
		$QUERY = "SELECT MAX(Tech_doc_id) FROM tech_doc";
		$result=mysqli_query($link, $QUERY) or die("Ошибка удаления ".mysqli_error($link));
		$last_doc_id = mysqli_fetch_row($result);
		$QUERY = "ALTER TABLE tech_doc AUTO_INCREMENT=".$last_doc_id[0];
		mysqli_query($link, $QUERY) or die("Ошибка сброса счетчика".mysqli_error($link));
...
if ($_FILES['userfile']['name']!=""){
		...
		$_FILES['userfile']['name'] = "tech_doc_n".$_GET["doc"].substr($my_string, $point_place, $type_len);
		...
		$QUERY = "UPDATE tech_doc SET Comments='".$_POST['comment']."', Type_doc='".$_POST['doc_type']."' WHERE Tech_doc_id=".$_GET["doc"];
		mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
		header("Location: gen_dir_cont_prep?n=".$_GET["n"]);
		exit;
	}
...
<head>
	...
	<title>Редактировать технический документ</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Редактировать технический документ</p>
						</div>
					</div>
<!--############################################################################# -->
						<form action="" method="POST" enctype="multipart/form-data">
							...
							<div class="row mt-4 mr-5 align-items-center" style="">
								...
								<input required class="col form-control" name="doc_type" type="text" value="<?echo $document[2];?>" placeholder="введите тип документа" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;">
								<div class="col-md-3 ml-4">	</div>
							...
							<div class="row mt-3 mr-5 align-items-center" style="">
								...
								<label class="col" for="upload-file" id="upload-file-label" style="height:32px; border: 1px solid rgba(0,0,0,.40); background: rgba(255,255,255,.20); font-size: small; padding-left: 1em; padding-top:0.5em;">Заменить</label>
							...
							<div class="row mt-3 mr-5 align-items-center" style="">
								...
								<input class="col form-control" name="comment" type="text" value="<?echo $document[1];?>" placeholder="комментарий" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;">
								<div class="col-md-3 ml-4"></div>
							</div>
							<div class="row mt-3 mr-5 align-items-center" style="">
						...
						<div class="row mt-3 mr-5 align-items-center" style="">
								<div class="col-md-3 text-right" style=""></div>
								<div class="col-md-2 p-0" name="cust_id">
									<button class="btn btn-danger btn-sm btn-block m-0" type="submit" name="delete"><small>Удалить</small></button>
								</div>
						</div>
						</form>
<!--############################################################################# -->
