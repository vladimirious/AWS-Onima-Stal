...
$QUERY= "SELECT 
	...
	WHERE 
		Proj_id=".$_GET["n"]." AND Stat_4=0";
$result = mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
if($result)
{
	$rows = mysqli_num_rows($result); // количество полученных строк
//	echo $rows;
		if($rows==0) echo '<button class="btn btn-primary btn-sm ml-0" onClick=\'location.href="otdel_zakupok_new_purchase?n='.$_GET["n"].'"\'><small>Добавить</small></button>';
		else{
			...
			for ($i = 0 ; $i < $rows ; ++$i)
			{
				...
				//print_r($result);
				echo "<tr><td>".$row[0]."</td>";
				echo "<td><a href='otdel_zakupok_prov?n=".$_GET["n"]."&prov=".$row[8]."'>".$row[1]."</a></td>";
				echo "<td>".$row[2]."</td>";
				echo "<td>".$row[3]."</td>";
				...
				<form method='POST' action=''>
					<select id='stage' name='stage' style='border: 0px; background-color: rgba(0,0,0,0);'>
						...
						<option value=".$row[0].">Доставляется</option>
					</select>
				...
				else if ($row[6]==0) echo "<td class='text-center'><a href='otdel_zakupok_edit_purchase?n=".$_GET["n"]."&doc=".$row[0]."' style='color:#DA3B01' title='Нажмите, чтобы перейти к редактированию закупки.'>Нет</a></td>";
			...
			echo '<button class="col-md-2 ml-0 btn btn-primary btn-sm" onClick=\'location.href="otdel_zakupok_new_purchase?n='.$_GET["n"].'"\' style="margin-left: 2em"><small>Добавить</small></button>';
		}
}
?>
						</div>
