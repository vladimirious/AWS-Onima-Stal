...
if(isset($_POST['submit'])){
	...
	if ($_FILES['userfile']['name']!=""){
		...
		if(move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile))
		{
			...
			else $QUERY = "UPDATE contracts SET Contr_N=".$_POST['contract_num'].", Comments='".$_POST['comment']."', Link='".$uploadfile."' WHERE Proj_id=".$_GET["n"];
		...
		if($_POST['signed']!="")	
		$QUERY = "UPDATE contracts SET Contr_N=".$_POST['contract_num'].", Comments='".$_POST['comment']."', Stat_2=".$_POST['signed'].", DATE='".date("Y-m-d")."' WHERE Proj_id=".$_GET["n"];
		else $QUERY = "UPDATE contracts SET Contr_N=".$_POST['contract_num'].", Comments='".$_POST['comment']."' WHERE Proj_id=".$_GET["n"];
		mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
		mysqli_free_result($result);
		header("Location: gen_dir_cont_prep?n=".$_GET["n"]);
		exit;
...
<!--############################################################################# -->
						<form action="" method="POST" enctype="multipart/form-data">
							<div class="row mt-5 mr-5 align-items-center" style="">
								...
								<input required class="col form-control" name="contract_num" type="number" value="<?echo $contract[0];?>"placeholder="введите номер договора" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;">
							...
							<div class="row mt-3 mr-5 align-items-center" style="">
								...
								<select <?if(($contract[3]==0)||($contract[5]==1))echo "disabled".' title="Доступно после проверки договора"';?> class="col text-left pr-3 form-control"  name="signed" id="signed" style="height:38px; border: 1px solid #707070; background-color: rgba(255,255,255,.20);font-size: small;">
									<option <?if($contract[4]==0) echo "selected";?> value ="0" >Нет</option>";
									<option <?if($contract[4]==1) echo "selected";?> value ="1" >Да</option>";
								</select>
								<div class="col-md-3 ml-4"></div>
							</div>
