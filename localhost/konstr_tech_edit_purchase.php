...
$QUERY= "SELECT supplies.Prov_id, Numb, supplies.Comments, providers.Prov_name, supplies.Link FROM supplies LEFT JOIN providers ON supplies.prov_id=providers.prov_id WHERE Supp_id=".$_GET["doc"];
...
if(isset($_POST['submit']))
{
	...
	//echo $my_string."<BR>";
		...
		$_FILES['userfile']['name'] = "purchase_n".$_GET["doc"].substr($my_string, $point_place, $type_len);
		$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
	//echo $uploadfile;
		if(move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile))
		{
			
			$QUERY = "UPDATE supplies SET Stat_1=default, Numb='".$_POST['number']."', Comments='".$_POST['comment']."', Link='".$uploadfile."' WHERE Supp_id=".$_GET["doc"]." AND Proj_id=".$_GET["n"];
			...
			$QUERY = "UPDATE supplies SET Stat_1=default, Numb='".$_POST['number']."', Comments='".$_POST['comment']."' WHERE Supp_id=".$_GET["doc"]." AND Proj_id=".$_GET["n"];
			mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
			header("Location: konstr_tech_ktp?n=".$_GET["n"]);
			exit;
	}
...
<?php
	...
	{	
		...
		if ($rows){
			for ($i = 0 ; $i < $rows ; ++$i)
			{
				$prov = mysqli_fetch_row($result);
				//print_r($result);
				if($i==0)echo "<option value =".$prov[0].">".$prov[1]."</option>";
				else echo "<option value = ".$prov[0].">".$prov[1]."</option>";
			}
			mysqli_free_result($result);
		}
	}
?>
					...
					<div class="row mt-4 mr-5 align-items-center" >
						...
						<input required class="col form-control" value="<?echo $row[1];?>" name="number" type="text" placeholder="количество" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;" >
