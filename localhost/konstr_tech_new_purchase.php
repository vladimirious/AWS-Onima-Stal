...
if(isset($_POST['submit'])){
	$QUERY = "INSERT INTO supplies (Prov_id, Numb, Comments, Stat_4, Proj_id) VALUES (".$_POST['prov_id'].", '".$_POST['number']."', '".$_POST['comment']."', 1, ".$_GET["n"].")";
	mysqli_query($link, $QUERY) or die("Ошибка 1 ".mysqli_error($link));
	
	$QUERY = "SELECT MAX(Supp_id) FROM supplies WHERE Stat_4=1 AND Proj_id=".$_GET["n"];
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Новый заказ</p>
						</div>
					</div>
<!--############################################################################# -->
...
?>
						...
						<button class="col-md-3 btn btn-secondary btn-sm ml-4 mr-0" onClick='location.href="konstr_tech_new_prov?n=<?echo $_GET["n"];?>"' type="button" style="text-transform:none">Добавить нового поставщика</button>
					...
					<div class="row mt-4 mr-5 align-items-center" >
						...
						<input required class="col form-control" id="username" name="number" type="text" placeholder="информация" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;" >
