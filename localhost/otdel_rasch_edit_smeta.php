...
$QUERY = "SELECT Comments, Link FROM pr_contract_docs WHERE Pr_doc_id=".$_GET["doc"];
...
if(isset($_POST['submit'])){
	...
	if ($_FILES['userfile']['name']!=""){
		...
		$_FILES['userfile']['name'] = "smeta_n".$_GET["n"].substr($my_string, $point_place, $type_len);
		
		$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
		if(move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile))
		{
			$QUERY = "UPDATE pr_contract_docs SET Comments='".$_POST['comment']."', Link='".$uploadfile."' WHERE Pr_doc_id=".$_GET["doc"];
			mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
			header("Location: otdel_rasch_cont_prep?n=".$_GET["n"]);
			exit;
		}
		else echo "Ошибка!";
	}
	//файл не заменен
		...
		$QUERY = "UPDATE pr_contract_docs SET Comments='".$_POST['comment']."' WHERE Pr_doc_id=".$_GET["doc"];
		mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
		header("Location: otdel_rasch_cont_prep?n=".$_GET["n"]);
		exit;
	}
...
<head>
	...
	<title>Редактирование сметы</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Редактирование сметы</p>
						</div>
					</div>
<!--############################################################################# -->
						<form action="" method="POST" enctype="multipart/form-data">
							...
							<div class="row mt-3 mr-5 align-items-center" style="">
								...
								<input class="col form-control" name="comment" type="text" value="<?echo $smeta[0];?>" placeholder="комментарий" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;">
								<div class="col-md-3 ml-4"></div>
							</div>
