...
if(isset($_POST['submit'])){
	$QUERY = "INSERT INTO konstr_doc (Comments, Prov_id, Proj_id) VALUES ('".$_POST['comment']."', ".$_POST["prov_id"].", ".$_GET["n"].")";
	mysqli_query($link, $QUERY) or die("Ошибка 1 ".mysqli_error($link));
	
	$QUERY = "SELECT MAX(Konstr_doc_id) FROM konstr_doc WHERE Proj_id=".$_GET["n"];
	$result = mysqli_query($link, $QUERY) or die("Ошибка 2".mysqli_error($link));
	$konstr_doc_id = mysqli_fetch_row($result);
	$konstr_doc_id = $konstr_doc_id[0];
	mysqli_free_result($result);
	
	$uploaddir = 'files/konstr_docs/';
	...
	$_FILES['userfile']['name'] = "konstr_doc_n".$konstr_doc_id.substr($my_string, $point_place, $type_len);
	$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
	//echo $uploadfile;
	move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile);
	
	$QUERY = "UPDATE konstr_doc SET Link='".$uploadfile."' WHERE Konstr_doc_id =".$konstr_doc_id;
...
<head>
	...
	<title>Новый конструкторский документ</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Новый конструкторский документ</p>
						</div>
					</div>
<!--############################################################################# -->
...
<?
	$QUERY ="SELECT supplies.Prov_id, supplies.Supp_id, providers.Prov_name, supplies.Numb FROM supplies LEFT JOIN providers ON supplies.Prov_id=providers.Prov_id WHERE Stat_3=1 AND Stat_4=1;
	";
	...
	if($result)
	{
		...
		if ($rows){
			for ($i = 0 ; $i < $rows ; ++$i)
			{
				...
				//print_r($result);
				echo "<option value =".$row[0].">Заказ ".$row[1]." - ".$row[2]." - ".$row[3]."</option>";
			}
			mysqli_free_result($result);
		}
	}
?>
						...
						<div class="col-md-2 ml-4 mr-0"></div>
					</div>
