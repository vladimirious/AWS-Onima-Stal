...
if(isset($_POST['submit'])){
	$QUERY = "INSERT INTO techn_doc (Comments, Proj_id) VALUES ('".$_POST['comment']."', ".$_GET["n"].")";
	mysqli_query($link, $QUERY) or die("Ошибка 1 ".mysqli_error($link));
	
	$QUERY = "SELECT MAX(Techn_doc_id) FROM techn_doc WHERE Proj_id=".$_GET["n"];
	$result = mysqli_query($link, $QUERY) or die("Ошибка 2".mysqli_error($link));
	$techn_doc_id = mysqli_fetch_row($result);
	$techn_doc_id = $techn_doc_id[0];
	...
	$_FILES['userfile']['name'] = "techn_doc_n".$techn_doc_id.substr($my_string, $point_place, $type_len);
	$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
	//echo $uploadfile;
	move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile);
	
	$QUERY = "UPDATE techn_doc SET Link='".$uploadfile."' WHERE Techn_doc_id =".$techn_doc_id;
...
<head>
	...
	<title>Новый технологический документ</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Новый технологический документ</p>
						</div>
					</div>
<!--############################################################################# -->
