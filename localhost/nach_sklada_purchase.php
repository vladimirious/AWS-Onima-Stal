...
{	
	...
	if ($_POST['stage']!=""){
		$QUERY="UPDATE supplies SET Stat_3=1 WHERE Supp_id=".$_POST['stage']." AND Proj_id=".$_GET["n"];
		mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
		
		$QUERY= "INSERT INTO mater_for_proj (Numb, Comments, Arr_date, Proj_id) VALUES ((SELECT Numb FROM supplies WHERE Supp_id=".$_POST['stage']."), (SELECT Comments FROM supplies WHERE Supp_id=".$_POST['stage']."), CURDATE(), ".$_GET["n"].")";
		mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
...
$QUERY= "SELECT 
	...
	WHERE 
		Proj_id=".$_GET["n"]." AND Stat_4=0 AND Stat_2=1";
$result = mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
if($result)
{
	$rows = mysqli_num_rows($result); // количество полученных строк
//	echo $rows;
		if($rows==0) echo '<div style="color: black; opacity: 0.3">Доставляемых закупок не найдено</div>';
		else{
			...
			for ($i = 0 ; $i < $rows ; ++$i)
			{
				...
				//print_r($result);
				echo "<tr><td>".$row[0]."</td>";
				echo "<td><a href='nach_sklada_prov?n=".$_GET["n"]."&prov=".$row[8]."'>".$row[1]."</a></td>";
				echo "<td>".$row[2]."</td>";
				echo "<td>".$row[3]."</td>";
				if (($row[4]==1)&&($row[5]==0)) echo "<td>
				<form method='POST' action=''>
					<select id='stage' name='stage' style='border: 0px; background-color: rgba(0,0,0,0);'>
						<option selected value=''>Доставляется</option>
						<option value=".$row[0].">Получено</option>
					</select>
				<input type='submit' name='submit' value='Submit' style='display:none;'/>
				</form>
				</td>";
				else if (($row[4]==1)&&($row[5]==1)) echo "<td>Получено</td>";

				
				if($row[6]=="") echo "<td class='text-center' style='color:#FF8C00'>Ожидается</td>";
				else if ($row[6]==0) echo "<td class='text-center'><a href='nach_sklada_edit_purchase?n=".$_GET["n"]."&doc=".$row[0]."' style='color:#DA3B01' title='Нажмите, чтобы перейти к редактированию закупки.'>Нет</a></td>";
