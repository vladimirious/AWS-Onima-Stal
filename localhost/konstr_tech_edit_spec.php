...
$QUERY= "SELECT Link, Comments FROM specifications WHERE Proj_id=".$_GET["n"];
...
if(isset($_POST['submit']))
{
	...
	//echo $my_string."<BR>";
		...
		$_FILES['userfile']['name'] = "spec_n".$_GET["doc"].substr($my_string, $point_place, $type_len);
		$uploadfile = $uploaddir.basename($_FILES['userfile']['name']);
	//echo $uploadfile;
		if(move_uploaded_file($_FILES['userfile']['tmp_name'], $uploadfile))
		{
			
			$QUERY = "UPDATE supplies SET Comments='".$_POST['comment']."', Link='".$uploadfile."' WHERE Proj_id=".$_GET["n"];
			...
			$QUERY = "UPDATE supplies SET Comments='".$_POST['comment']."' WHERE AND Proj_id=".$_GET["n"];
			mysqli_query($link, $QUERY) or die("Ошибка ".mysqli_error($link));
			header("Location: konstr_tech_ktp?n=".$_GET["n"]);
			exit;
	}
...
<head>
	...
	<title>Редактирование спецификации</title>
    <!-- Bootstrap core CSS -->
			...
			<div class="col-fluid">
				<div class="main-w" style="width: 980px; height:661px;">
					...
					<div class="row-fluid ml-3 ">
						<div class="col">
							<p class="sh1 mb-1">Редактирование спецификации заказа №<?echo $_GET["n"]?></p>
						</div>
					</div>
<!--############################################################################# -->
					...
					<div class="row mt-4 mr-5 align-items-center" style="">
						...
						<label class="col" for="upload-file" id="upload-file-label" style="height:35px; border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small; padding-left: 10px; padding-top:8px;">Обзор...</label>
					...
					<div class="row mt-4 mr-5 align-items-center" style="">
						...
						<input required class="col form-control" id="username" value="<?echo $row[1];?>" name="comment" type="text" placeholder="комментарий" style="border: 1px solid rgba(0,0,0,.40); background-color: rgba(255,255,255,.20); font-size: small;" >
					...
					<div class="row mt-4 mr-5 align-items-center" style="">
						...
						<div class="col pl-0">
							<button class="btn btn-primary btn-sm ml-0" type="submit" name="submit">Сохранить</button>
						</div>
						<div class="col">
						</div>
					</div>
